...
Run:
    python phase_2d_xbid_continuous_intraday/price_and_power_forecasting/xbid_price_train_val_test.py
"""
...
def _write_report(selected: str, cv_mae: dict, test_mae: float, naive_mae: float,
    ...
    lines += [
        f"",
        f"## XBID Gate (Production)",
        f"- Tradable hours: H1-H24 (gate closes 1h before each delivery period)",
        f"- Two check windows: W1 (D-1 18:30 CET), W2 (D 09:30 CET)",
        f"- Per-order cap: xbid_max_volume_per_order_mw (config)",
        f"- Order placed only if avg gain > xbid_min_spread_eur_mwh (no-churn)",
        f"- Spread std wider than IDA3 (~14 vs ~11 EUR/MWh): closer to delivery",
    ]
