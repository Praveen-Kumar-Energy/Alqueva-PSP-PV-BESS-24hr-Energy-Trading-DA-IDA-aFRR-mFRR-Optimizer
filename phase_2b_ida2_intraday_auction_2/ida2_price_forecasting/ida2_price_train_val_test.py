"""
ida2_price_train_val_test.py — offline evaluation of the IDA2 lead-time spread model.
...
Run:
    python phase_2b_ida2_intraday_auction_2/ida2_price_forecasting/ida2_price_train_val_test.py
"""
...
_REPORT     = os.path.join(_HERE, "ida2_price_evaluation_report.md")
_N_FOLDS    = 4
_TEST_YEAR  = 2025
...
def evaluate_ida() -> None:
    ...
    buckets = {
        "Off-peak (H3-H6, H23-H24)": list(range(3, 7)) + [23, 24],
...
def _write_report(selected: str, cv_mae: dict, test_mae: float, naive_mae: float,
    ...
    lines += [
        f"",
        f"## IDA2 Gate (Production)",
        f"- Tradable hours: H3-H24 (H1-H2 frozen after IDA1; gate closes D-1 22:00 CET)",
        f"- Dedicated model trained on IDA2 SIDC clearing prices only",
        f"- Baseline for IDA3 re-optimisation",
    ]
