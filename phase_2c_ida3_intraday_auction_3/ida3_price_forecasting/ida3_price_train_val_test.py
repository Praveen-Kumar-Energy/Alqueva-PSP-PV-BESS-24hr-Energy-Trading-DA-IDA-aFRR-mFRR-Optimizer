"""
ida3_price_train_val_test.py — offline evaluation of the IDA3 lead-time spread model.
...
Naive baseline for spread forecasting: spread = 0  (IDA3 price = DA price).
...
Run:
    python phase_2c_ida3_intraday_auction_3/ida3_price_forecasting/ida3_price_train_val_test.py
"""
...
_REPORT     = os.path.join(_HERE, "ida3_price_evaluation_report.md")
_N_FOLDS    = 4
_TEST_YEAR  = 2025
...
def evaluate_ida() -> None:
    ...
    # Per-hour-bucket test breakdown (IDA3 covers H12-H24 only)
    print(f"\nPer-hour-bucket test MAE (EUR/MWh spread):")
    test["pred_spread"] = preds
    buckets = {
        "Midday   (H12-H16)": list(range(12, 17)),
        "Evening  (H17-H24)": list(range(17, 25)),
    }
...
def _write_report(selected: str, cv_mae: dict, test_mae: float, naive_mae: float,
    ...
    lines = [
        ...
        f"",
        f"## Data",
        f"- Source: `ida3_training_data_2024_2025.xlsx` (OMIE/ENTSO-E SIDC intraday results)",
        f"- Range : 2024-06-13 to {excel_last}",
        f"- Gate   : IDA3 (H12-H24, closes D 10:00 CET; H1-H11 frozen after IDA1/IDA2)",
    ...
    lines += [
        f"",
        f"## IDA3 Gate (Production)",
        f"- Tradable hours: H12-H24 (H1-H11 frozen after IDA1/IDA2; gate closes D 10:00 CET)",
        f"- Dedicated model trained on IDA3 SIDC clearing prices only",
        f"- Final intraday gate; no further re-optimisation after this",
    ]
